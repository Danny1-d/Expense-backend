...
export const AuthMiddleware = (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    const decoded = jwt.verify(token, process.env.JWT_SECRET as string);// Verify the token using the secret key
    ...
    if (typeof decoded === "object" && "id" in decoded) {
      req.body.userId = (decoded as JwtPayload).id;// Attach user ID to the request body
    } else {
      res.status(401).json({ message: "Not Authorized. Login again" });
    }
