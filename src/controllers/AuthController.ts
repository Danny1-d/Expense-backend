    ...
    if (checkResult.rows.length > 0) {
      ...
      bcrypt.hash(password, saltRounds, async (err, hash) => {
        if (err) {
          ...
          const result = await db.query("INSERT INTO users (firstName, lastName, email, password) VALUES ($1, $2, $3, $4) RETURNING *", [firstName, lastName, email, hash]);
      ...
      bcrypt.compare(password, storedPassword, async (err, result) => {
        if (err) {
          ...
          if (result) {
            ...
            const token = Jwt.sign({ id: newUser }, process.env.JWT_SECRET as string, { expiresIn: "1h" });
...
export const verifyEmail = async (req:Request, res:Response) => {
    ...
    await db.query("UPDATE users SET isAccountVerified = true, verifyOtp = NULL, verifyOtpExpireAt = NULL WHERE id = $1", [userId]);
...
export const resetPassword = async (req:Request, res:Response) => {
  ...
  try {
    ...
    bcrypt.hash(newPassword, saltRounds, async (err, hash) => {
      ...
      await db.query("UPDATE users SET password = $1, resetOtp = NULL, resetOtpExpiresAt = NULL WHERE email = $2", [hash, email]);
